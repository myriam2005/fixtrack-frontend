...
import DetailTicket from "../employee/DetailsTickets"; // ✅ ajout
...
// ── TicketRow ─────────────────────────────────────────────────────────────────
function TicketRow({ ticket, isLast, onVoirDetails }) { // ✅ onVoirDetails
    ...
    <>
      <Box sx={{
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography sx={{
            fontWeight: 600, fontSize: { xs: "13px", sm: "13.5px" }, color: "#111827",
            whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", mb: "3px",
          }}>
            {ticket.titre}
          </Typography>
        ...
        {/* ✅ Voir détails → ouvre le modal */}
        <Box
          onClick={() => onVoirDetails(ticket.id)}
          sx={{
            display: "flex", alignItems: "center", gap: "4px",
            flexShrink: 0, cursor: "pointer", color: "#2563EB", fontWeight: 600,
            whiteSpace: "nowrap", alignSelf: { xs: "flex-start", sm: "center" },
            mt: { xs: "2px", sm: 0 }, transition: "all 0.15s",
            "&:hover": { color: "#1D4ED8", gap: "6px" },
          }}
        >
          <Typography sx={{ fontSize: "13px", fontWeight: 600, color: "inherit", display: { xs: "none", sm: "block" } }}>
            Voir détails
          </Typography>
...
export default function TechnicianDashboard() {
  ...
  const [selectedId,   setSelectedId]   = useState(null); // ✅ ajout
        ...
        <Box sx={{ padding: { xs: "12px 14px 0", sm: "16px 24px 0" }, borderBottom: "1px solid #F3F4F6" }}>
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: "12px" }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: "4px", color: "#9CA3AF" }}>
              ...
              <Typography sx={{ fontSize: "11px", fontWeight: 600, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.06em", display: { xs: "none", sm: "block" } }}>
                Filtrer
              </Typography>
            </Box>
          </Box>
            ...
            {FILTER_TABS.map((tab) => {
              ...
              return (
                <Box key={tab.key} onClick={() => setActiveFilter(tab.key)} sx={{
                  ...
                  <Typography sx={{ fontSize: { xs: "12px", sm: "13px" }, fontWeight: isActive ? 700 : 500, color: isActive ? "#2563EB" : "#6B7280", whiteSpace: "nowrap" }}>
                    {tab.label}
                  </Typography>
          ...
          {filteredTickets.length === 0 ? (
            ...
            filteredTickets.map((ticket, index) => (
              <TicketRow
                ...
                onVoirDetails={(id) => setSelectedId(id)} // ✅ ouvre le modal
              />
            ))
          )}
        </Box>
