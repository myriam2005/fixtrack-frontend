...
// Accessible par : technician | manager | admin
// Export téléchargeable : PDF (jsPDF + autoTable), Excel (SheetJS), CSV natif

import { useState, useMemo } from "react";
import { useAuth } from "../context/AuthContext";
import { tickets, users } from "../data/mockData";

// ── Shared components (no more local duplicates) ──────────────────────────────
...
// Priority bubble cards (not Badge — these are big number cards)
const PRIORITY_META = {
...
const getName = (id) => users.find(u => u.id === id)?.nom || "—";

// ─── Dates min/max calées sur mockData ────────────────────────────────────────
const ALL_DATES     = tickets.map(t => t.dateCreation).sort();
const DATA_DATE_MIN = ALL_DATES[0]                    || "2025-01-01";
const DATA_DATE_MAX = ALL_DATES[ALL_DATES.length - 1] || new Date().toISOString().slice(0,10);

// ─── Compute report ───────────────────────────────────────────────────────────
function computeReport(type, userId, userRole, dateFrom, dateTo) {
  ...
  if (type === "tickets") {
    ...
    return {
      ...
      summary: {
        total: src.length,
        open: byStatus.open||0, assigned: byStatus.assigned||0,
        inProgress: byStatus.in_progress||0,
        resolved: byStatus.resolved||0, closed: byStatus.closed||0,
      },
      byPriority, byCategory,
      rows: src.map(t => ({
        id:t.id, titre:t.titre, statut:t.statut, priorite:t.priorite,
        categorie:t.categorie, localisation:t.localisation,
        technicien:getName(t.technicienId), date:t.dateCreation,
      })),
    };
  }
  ...
  if (type === "performance") {
    ...
    return {
      type,
      rows: techs.map(tech => {
        const assigned = src.filter(t => t.technicienId === tech.id);
        ...
        return {
          id:tech.id, nom:tech.nom,
          competences:tech.competences?.join(", ")||"—",
          assigned:assigned.length, resolved:resolved.length,
          inProgress:inProg.length, rate,
        };
      }),
    };
  }
  ...
  if (type === "worklog") {
    ...
    return {
      type,
      rows: filtered.map(t => ({
        id:t.id, titre:t.titre, technicien:getName(t.technicienId),
        statut:t.statut, categorie:t.categorie, localisation:t.localisation,
        date:t.dateCreation, notes:t.notes?.[0]||"—",
      })),
    };
  }
  ...
  if (type === "audit") {
    ...
    return {
      ...
      byRole,
      rows: src.map(t => ({
        id:t.id, titre:t.titre, auteur:getName(t.auteurId),
        technicien:getName(t.technicienId), statut:t.statut,
        priorite:t.priorite, date:t.dateCreation,
      })),
    };
  }
  return null;
...
function getExportData(report) {
  ...
  if (report.type === "tickets") return {
    cols: ["ID","Titre","Statut","Priorité","Catégorie","Localisation","Technicien","Date"],
    rows: report.rows.map(r => [r.id, r.titre, r.statut, r.priorite, r.categorie, r.localisation, r.technicien, r.date]),
  };
  if (report.type === "performance") return {
    cols: ["ID","Technicien","Compétences","Assignés","Résolus","En cours","Taux (%)"],
    rows: report.rows.map(r => [r.id, r.nom, r.competences, r.assigned, r.resolved, r.inProgress, r.rate]),
  };
  if (report.type === "worklog") return {
    cols: ["ID","Titre","Technicien","Statut","Catégorie","Localisation","Date","Notes"],
    rows: report.rows.map(r => [r.id, r.titre, r.technicien, r.statut, r.categorie, r.localisation, r.date, r.notes]),
  };
  if (report.type === "audit") return {
    cols: ["ID","Titre","Auteur","Technicien","Statut","Priorité","Date"],
    rows: report.rows.map(r => [r.id, r.titre, r.auteur, r.technicien, r.statut, r.priorite, r.date]),
  };
  return { cols: [], rows: [] };
}
...
async function exportPDF(report, filename, title, description, period) {
  ...
  doc.text("FixTrack — " + title, 14, 18);
  doc.setFontSize(10); doc.setTextColor(107,114,128);
  doc.text(`Généré le ${new Date().toLocaleString("fr-FR")}   |   Période : ${period}`, 14, 26);
  let startY = 32;
  if (description) {
    doc.setFontSize(10); doc.setTextColor(30,30,30);
    doc.text("Description : " + description, 14, 34);
    startY = 42;
  }
  const { cols, rows } = getExportData(report);
...
// ─── Local-only micro-composants (pas de doublon avec les shared) ─────────────

// KpiCard : pas de version shared — reste local
...
// SectionTitle : divider with label — no shared equivalent
...
// DataTable : no shared equivalent
      ...
      <table style={{ width:"100%", borderCollapse:"collapse", minWidth:480 }}>
        <thead>
          <tr style={{ background:"#f9fafb", borderBottom:`1px solid ${BORDER}` }}>
            {cols.map(c=>(
              <th key={c} style={{ padding:"9px 14px", textAlign:"left", fontSize:11, fontWeight:700, color:MUTED, textTransform:"uppercase", letterSpacing:".5px", whiteSpace:"nowrap" }}>{c}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((row,i)=>(
            <tr key={i}
              style={{ borderBottom:`1px solid ${BORDER}`, background:hov===i?"#f9fafb":"transparent", transition:"background .1s" }}
              onMouseEnter={()=>setHov(i)} onMouseLeave={()=>setHov(-1)}>
              {row.map((cell,j)=>(
                <td key={j} style={{ padding:"10px 14px", fontSize:12.5, color:TEXT, verticalAlign:"middle" }}>{cell}</td>
              ))}
            </tr>
          ))}
        </tbody>
...
// ─── ReportViewer ─────────────────────────────────────────────────────────────
// Badge remplace l'ancien Pill local — même API de statut/priorité
function ReportViewer({ report, generatedAt, description }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      {description && (
        <div style={{ padding:"10px 14px", background:"#f8fafc", border:`1px solid ${BORDER}`, borderRadius:8, fontSize:13, color:TEXT, fontStyle:"italic" }}>
          📝 {description}
        </div>
      )}
      <div style={{ fontSize:11, color:MUTED }}>
        Généré le {dateStr} · {report.rows?.length ?? 0} ligne(s)
      </div>
      ...
      {report.type === "tickets" && <>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {Object.entries(report.byPriority).map(([p,n])=>{
            ...
            return (
              <div key={p} style={{ padding:"10px 16px", borderRadius:10, background:m.bg||"#f3f4f6", border:`1px solid ${m.border||BORDER}`, display:"flex", alignItems:"center", gap:8 }}>
        ...
        <DataTable
          ...
          rows={report.rows.map(r=>[
            <span style={{ fontFamily:"monospace", fontSize:11.5 }}>{r.id}</span>,
...
export default function ReportsPage() {
  ...
  const [dateFrom,     setDateFrom]     = useState(DATA_DATE_MIN);
  const [dateTo,       setDateTo]       = useState(DATA_DATE_MAX);
  ...
  const quickStats = useMemo(() => ({
    ...
    ready:     history.filter(h=>h.report).length,
  ...
  const handleGenerate = async () => {
    ...
    const now    = new Date();
    const report = computeReport(selectedType, userId, role, dateFrom, dateTo);
      ...
      <style>{`
        ...
        * { box-sizing:border-box; }
        ...
        @media (max-width:900px) {
          .rp-page { padding:24px 20px; }
          .rp-kpi-grid   { grid-template-columns:repeat(2,1fr); gap:10px; margin-bottom:20px; }
          .rp-audit-grid { grid-template-columns:repeat(2,1fr); }
          .rp-config-grid { grid-template-columns:1fr 1fr; }
          .rp-drawer { width:320px; }
        }

        /* ── MOBILE ≤ 600px ── */
        @media (max-width:600px) {
          .rp-page { padding:16px 14px; }
          .rp-header { flex-direction:column; align-items:stretch; margin-bottom:20px; }
          .rp-header h1 { font-size:20px !important; }
          .rp-hist-btn { align-self:flex-start; }
          .rp-kpi-grid   { grid-template-columns:1fr 1fr; gap:8px; margin-bottom:16px; }
          .rp-audit-grid { grid-template-columns:1fr 1fr; gap:8px; }
          .rp-config-grid { grid-template-columns:1fr; gap:14px; }
          .rp-period-row  { flex-direction:column; gap:8px; }
          .rp-period-row input { width:100% !important; }
          .rp-gen-wrap { justify-content:stretch; }
          .rp-gen-btn  { width:100%; justify-content:center; padding:0 16px; }
          .rp-result-header { flex-direction:column; align-items:flex-start; }
          .rp-result-actions { width:100%; }
          .rp-result-actions button:first-child { flex:1; justify-content:center; }
          .rp-drawer { width:100%; border-left:none; border-top:1px solid ${BORDER}; top:auto; height:85vh; border-radius:16px 16px 0 0; animation:slideUp .25s ease; }
        ...
        @media (max-width:380px) {
          ...
          .rp-audit-grid { grid-template-columns:1fr; }
          .rp-fmt-row    { flex-direction:column; }
          .rp-fmt-row button { width:100%; flex:none !important; }
        }
      `}</style>

      {/* Toast */}
      {toast && (
        <div style={{ position:"fixed", bottom:28, right:28, zIndex:9999, background:toast.isError?"#dc2626":"#1e293b", color:"#fff", borderRadius:12, padding:"12px 22px", fontSize:13.5, fontWeight:600, boxShadow:"0 8px 32px rgba(0,0,0,0.22)", animation:"fadeUp .3s ease", maxWidth:"calc(100vw - 32px)" }}>
          {toast.msg}
        </div>
      )}
...
>
  Rapports Système
          ...
          {/* Bouton Historique — Button component */}
          <Button
            label={
              <span style={{ display:"flex", alignItems:"center", gap:2, fontSize:12}}>
                🕐 Historique
                {history.length > 0 && (
                  <span style={{ background:BLUE, color:"#fff", borderRadius:20, fontSize:8, padding:"1px 8px", fontWeight:700 }}>{history.length}</span>
                )}
              </span>
            }
        ...
        <div style={{ background:"#fff", border:`1px solid ${BORDER}`, borderRadius:14, padding:"24px 28px", marginBottom:24, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
            ...
            <div>
              ...
              <div className="rp-period-row">
                <input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)}
                  style={{ flex:1, height:44, border:`1px solid ${BORDER}`, borderRadius:9, padding:"0 10px", fontSize:13, color:TEXT }}/>
            ...
            <div>
              ...
              <div className="rp-fmt-row">
                {["pdf","excel","csv"].map(f => {
                  ...
                  return (
                    <button key={f} onClick={()=>setSelectedFmt(f)} style={{
                      flex:1, height:44, borderRadius:9, cursor:"pointer", fontFamily:"inherit",
                      fontSize:13, fontWeight:600, transition:"all .15s",
                      display:"flex", alignItems:"center", justifyContent:"center", gap:5,
                      border:`1.5px solid ${active?BLUE:BORDER}`,
                      background: active ? BLUE_L : "#fff",
          ...
          <div style={{ marginTop:18 }}>
            ...
            <textarea
              ...
              placeholder="Ajoutez une note ou un contexte pour ce rapport (ex : rapport mensuel janvier, audit suite incident…)"
              rows={2}
          ...
          {/* Bouton Générer — Button component */}
          <div className="rp-gen-wrap">
            <button
              ...
              disabled={generating}
              style={{
                background: generating ? "#93c5fd" : `linear-gradient(135deg,${BLUE} 0%,#1d4ed8 100%)`,
                boxShadow:  generating ? "none"     : "0 4px 16px rgba(37,99,235,0.28)",
                cursor:     generating ? "not-allowed" : "pointer",
              }}
              onMouseEnter={e=>{ if(!generating){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(37,99,235,0.38)";} }}
              onMouseLeave={e=>{ e.currentTarget.style.transform="";e.currentTarget.style.boxShadow=generating?"none":"0 4px 16px rgba(37,99,235,0.28)"; }}
            >
          ...
          <div style={{ background:"#fff", border:`1px solid ${BORDER}`, borderRadius:14, padding:"24px 28px", boxShadow:"0 1px 4px rgba(0,0,0,0.05)", animation:"fadeUp .4s ease" }}>
            <div className="rp-result-header">
              ...
              <div className="rp-result-actions">
                {/* Télécharger — bouton custom (gradient vert) */}
                <button onClick={handleDownload} disabled={downloading} style={{
                  height:40, padding:"0 20px",
                  background: downloading ? "#bbf7d0" : "linear-gradient(135deg,#16a34a 0%,#15803d 100%)",
                  border:"none", borderRadius:9, cursor:downloading?"not-allowed":"pointer",
                  color:"#fff", fontFamily:"inherit", fontSize:13, fontWeight:700,
                  boxShadow: downloading?"none":"0 3px 12px rgba(22,163,74,0.25)",
                  transition:"all .18s", display:"flex", alignItems:"center", gap:7, whiteSpace:"nowrap",
                }}
                  onMouseEnter={e=>{ if(!downloading){e.currentTarget.style.transform="translateY(-1px)";e.currentTarget.style.boxShadow="0 6px 18px rgba(22,163,74,0.35)";} }}
                  onMouseLeave={e=>{ e.currentTarget.style.transform="";e.currentTarget.style.boxShadow=downloading?"none":"0 3px 12px rgba(22,163,74,0.25)"; }}
                >
          ...
          <div className="rp-drawer">
              ...
              {history.length === 0 ? (
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  {history.map(entry => {
                    ...
                    return (
                      <div key={entry.id}
                        onClick={()=>{ setActiveReport(entry.report); setActiveGenAt(entry.generatedAt); setActiveDesc(entry.description||""); setSelectedFmt(entry.fmt); setShowHistory(false); showToast(`📂 ${TYPE_LABELS[entry.type]} chargé`); }}
                      ...
                      >
                        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                          ...
                          <div style={{ flex:1, minWidth:0 }}>
                            ...
                            {entry.description && (
                              <div style={{ fontSize:11, color:MUTED, fontStyle:"italic", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>📝 {entry.description}</div>
                            )}
            ...
            {history.length > 0 && (
              <div style={{ padding:"16px 20px", borderTop:`1px solid ${BORDER}` }}>
                {/* Effacer — Button danger */}
                <Button
                  label="🗑️ Effacer l'historique"
                  onClick={()=>{ setHistory([]); setActiveReport(null); setShowHistory(false); showToast("🗑️ Historique effacé."); }}
                  variant="danger"
                  fullWidth
                />
              </div>
