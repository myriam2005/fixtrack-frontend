...
// Fix: setState dans useEffect pour les dates → initialisé directement dans useState
...
function computeReport(type, userId, userRole, dateFrom, dateTo, tickets, users) {
  ...
  if (type === "tickets") {
    ...
    return {
      ...
      rows: src.map(t => ({ id: t._id||t.id, titre:t.titre, statut:t.statut, priorite:t.priorite, categorie:t.categorie, localisation:t.localisation, technicien:getName(t.technicienId, users), date:t.dateCreation })),
    };
  }
  if (type === "performance") {
  ...
  if (type === "worklog") {
    ...
    return { type, rows: filtered.map(t => ({ id:t._id||t.id, titre:t.titre, technicien:getName(t.technicienId, users), statut:t.statut, categorie:t.categorie, localisation:t.localisation, date:t.dateCreation, notes:t.notes?.[0]||"—" })) };
  }
  if (type === "audit") {
    const byRole={};
    ...
    return {
      ...
      byRole,
      rows: src.map(t => ({ id:t._id||t.id, titre:t.titre, auteur:getName(t.auteurId, users), technicien:getName(t.technicienId, users), statut:t.statut, priorite:t.priorite, date:t.dateCreation })),
    };
  }
  return null;
}
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      {report.type === "tickets" && <>
        ...
        <DataTable cols={["ID","Titre","Statut","Priorité","Catégorie","Technicien","Date"]}
          rows={report.rows.map(r=>[<span style={{ fontFamily:"monospace", fontSize:11.5 }}>{r.id}</span>,<div><div style={{ fontWeight:600 }}>{r.titre}</div><div style={{ fontSize:11, color:MUTED }}>{r.localisation}</div></div>,<Badge status={r.statut}/>,<Badge status={r.priorite}/>,r.categorie,r.technicien,r.date])}/>
      </>}
      {report.type === "performance" && (
        <DataTable cols={["Technicien","Compétences","Assignés","Résolus","En cours","Taux résolution"]}
          rows={report.rows.map(r=>[<span style={{ fontWeight:700 }}>{r.nom}</span>,<span style={{ fontSize:11.5, color:MUTED }}>{r.competences}</span>,<span style={{ fontWeight:700, color:BLUE }}>{r.assigned}</span>,<span style={{ fontWeight:700, color:"#16a34a" }}>{r.resolved}</span>,<span style={{ fontWeight:700, color:"#d97706" }}>{r.inProgress}</span>,<div style={{ display:"flex", alignItems:"center", gap:8 }}><div style={{ flex:1, height:8, background:"#e5e7eb", borderRadius:4, overflow:"hidden", minWidth:60 }}><div style={{ width:`${r.rate}%`, height:"100%", borderRadius:4, background:r.rate>=70?"#16a34a":r.rate>=40?"#d97706":"#ef4444" }}/></div><span style={{ fontSize:12.5, fontWeight:800, color:r.rate>=70?"#16a34a":r.rate>=40?"#d97706":"#ef4444", minWidth:34 }}>{r.rate}%</span></div>])}/>
      )}
      {report.type === "worklog" && (
        <DataTable cols={["ID","Titre","Technicien","Statut","Catégorie","Localisation","Date","Notes"]}
          rows={report.rows.map(r=>[<span style={{ fontFamily:"monospace", fontSize:11.5 }}>{r.id}</span>,<span style={{ fontWeight:600 }}>{r.titre}</span>,r.technicien,<Badge status={r.statut}/>,r.categorie,<span style={{ fontSize:11.5, color:MUTED }}>{r.localisation}</span>,r.date,<span style={{ fontSize:11.5, color:MUTED, fontStyle:"italic" }}>{r.notes}</span>])}/>
      )}
      {report.type === "audit" && <>
        ...
        <DataTable cols={["ID","Titre","Auteur","Technicien","Statut","Priorité","Date"]}
          rows={report.rows.map(r=>[<span style={{ fontFamily:"monospace", fontSize:11.5 }}>{r.id}</span>,<span style={{ fontWeight:600 }}>{r.titre}</span>,r.auteur,r.technicien,<Badge status={r.statut}/>,<Badge status={r.priorite}/>,r.date])}/>
      </>}
    </div>
  );
}
...
export default function ReportsPage() {
  ...
  // Fix: dates calculées via useMemo → initialisées directement dans useState via callback
  // On NE FAIT PAS de useEffect avec setState pour les dates
  const allDates = useMemo(
    () => tickets.map(t => t.dateCreation).filter(Boolean).sort(),
    [tickets]
  );

  // Fix: on initialise les dates avec une valeur par défaut statique
  // et on les met à jour via le setter quand allDates change,
  // MAIS on évite le setState dans un useEffect → on utilise un state séparé
  // qui se calcule directement depuis allDates (dérivé, pas d'effet)
  ...
  // Fix: on initialise avec les valeurs statiques et on les contrôle manuellement
  ...
  // Fix: on track si les dates ont déjà été initialisées depuis l'API
  const [datesInitialized, setDatesInitialized] = useState(false);

  // Fix: uniquement si pas encore initialisé et données disponibles
  useEffect(() => {
    ...
    <>
      <style>{`
        ...
        * { box-sizing:border-box; }
        ...
        @media (max-width:900px) { .rp-page{padding:24px 20px} .rp-kpi-grid{grid-template-columns:repeat(2,1fr);gap:10px;margin-bottom:20px} .rp-audit-grid{grid-template-columns:repeat(2,1fr)} .rp-config-grid{grid-template-columns:1fr 1fr} .rp-drawer{width:320px} }
        @media (max-width:600px) { .rp-page{padding:16px 14px} .rp-header{flex-direction:column;align-items:stretch;margin-bottom:20px} .rp-header h1{font-size:20px!important} .rp-kpi-grid{grid-template-columns:1fr 1fr;gap:8px;margin-bottom:16px} .rp-audit-grid{grid-template-columns:1fr 1fr;gap:8px} .rp-config-grid{grid-template-columns:1fr;gap:14px} .rp-period-row{flex-direction:column;gap:8px} .rp-period-row input{width:100%!important} .rp-gen-wrap{justify-content:stretch} .rp-gen-btn{width:100%;justify-content:center;padding:0 16px} .rp-result-header{flex-direction:column;align-items:flex-start} .rp-result-actions{width:100%} .rp-drawer{width:100%;border-left:none;border-top:1px solid ${BORDER};top:auto;height:85vh;border-radius:16px 16px 0 0;animation:slideUp .25s ease} @keyframes slideUp{from{transform:translateY(100%);opacity:0}to{transform:translateY(0);opacity:1}} }
        @media (max-width:380px) { .rp-kpi-grid{grid-template-columns:1fr} .rp-audit-grid{grid-template-columns:1fr} .rp-fmt-row{flex-direction:column} .rp-fmt-row button{width:100%;flex:none!important} }
      `}</style>

      {toast && <div style={{ position:"fixed", bottom:28, right:28, zIndex:9999, background:toast.isError?"#dc2626":"#1e293b", color:"#fff", borderRadius:12, padding:"12px 22px", fontSize:13.5, fontWeight:600, boxShadow:"0 8px 32px rgba(0,0,0,0.22)", animation:"fadeUp .3s ease", maxWidth:"calc(100vw - 32px)" }}>{toast.msg}</div>}

      <div className="rp-page">
        <div className="rp-header">
          <div>
            <h1 style={{ fontFamily:"'DM Serif Display', serif", fontSize:"clamp(30px, 5vw, 30px)", fontWeight:700, color:"#0F172A", letterSpacing:"-0.02em", lineHeight:1.2, margin:"0 0 6px" }}>Rapports Système</h1>
        ...
        <div style={{ background:"#fff", border:`1px solid ${BORDER}`, borderRadius:14, padding:"24px 28px", marginBottom:24, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
          ...
          <div className="rp-gen-wrap">
            <button className="rp-gen-btn" onClick={handleGenerate} disabled={generating||dataLoading}
              ...
              onMouseEnter={e=>{ if(!generating&&!dataLoading){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(37,99,235,0.38)";} }}
          ...
          <div style={{ background:"#fff", border:`1px solid ${BORDER}`, borderRadius:14, padding:"24px 28px", boxShadow:"0 1px 4px rgba(0,0,0,0.05)", animation:"fadeUp .4s ease" }}>
            <div className="rp-result-header">
              ...
              <div className="rp-result-actions">
                <button onClick={handleDownload} disabled={downloading} style={{ height:40, padding:"0 20px", background:downloading?"#bbf7d0":"linear-gradient(135deg,#16a34a 0%,#15803d 100%)", border:"none", borderRadius:9, cursor:downloading?"not-allowed":"pointer", color:"#fff", fontFamily:"inherit", fontSize:13, fontWeight:700, boxShadow:downloading?"none":"0 3px 12px rgba(22,163,74,0.25)", transition:"all .18s", display:"flex", alignItems:"center", gap:7, whiteSpace:"nowrap" }}
