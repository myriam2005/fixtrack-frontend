...
// ── Mapping nom → icône (pour les catégories dynamiques) ─────────────────────
const ICON_MAP = {
  "Électrique":        Icon.Zap,
  "Plomberie":         Icon.Droplet,
  "HVAC":              Icon.Wind,
  "Informatique":      Icon.Monitor,
  "Menuiserie/Mobilier": Icon.Armchair,
  "Hygiène/Nettoyage": Icon.Sparkles,
  "Sécurité":          Icon.Shield,
  "Mécanique":         Icon.Settings,
  "Autre":             Icon.MoreCircle,
};
...
export function useDynamicCategories() {
  ...
  useEffect(() => {
    api.get("/config/categories")
      .then(res => {
        const fromApi = (res.data || []).map(cat => ({
          value: cat.nom,
          IcoComp: ICON_MAP[cat.nom] || Icon.Generic,
        }));
        // Ajoute toujours "Autre" en dernier
        const hasAutre = fromApi.some(c => c.value === "Autre");
        if (!hasAutre) fromApi.push({ value: "Autre", IcoComp: Icon.MoreCircle });
        if (fromApi.length > 0) setCategories(fromApi);
      })
      .catch(() => {
        // Garde les défauts si l'API échoue
      });
  }, []);
...
// ── Export CATEGORIES statique pour compatibilité (fallback) ──────────────────
