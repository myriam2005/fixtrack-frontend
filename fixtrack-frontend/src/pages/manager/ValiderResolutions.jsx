...
const Icon = {
  ...
  note: (<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>),
  ...
  checkBig: (<svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>),
};

const PRIORITY_BORDER = { critical: "#EF4444", high: "#F59E0B", medium: "#3B82F6", low: "#D1D5DB" };
const FILTER_TABS = [
  { key: "all", label: "Tous" }, { key: "critical", label: "Critique" },
  { key: "high", label: "Haute" }, { key: "medium", label: "Moyenne" }, { key: "low", label: "Basse" },
];
...
function KpiCard({ icon, label, count, color, bgColor, description }) {
  return (
    <Paper elevation={0} sx={{ borderRadius: "14px", padding: "20px 22px", border: "1px solid #E5E7EB", backgroundColor: "#FFFFFF", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", position: "relative", overflow: "hidden", transition: "transform 0.2s, box-shadow 0.2s", "&:hover": { transform: "translateY(-3px)", boxShadow: "0 8px 24px rgba(0,0,0,0.09)" } }}>
      ...
      <Box sx={{ position: "relative", zIndex: 1, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <Box>
          <Typography sx={{ fontSize: "11px", fontWeight: 700, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.08em", mb: "8px" }}>{label}</Typography>
          <Typography sx={{ fontSize: "36px", fontWeight: 900, color: "#111827", lineHeight: 1, letterSpacing: "-0.04em" }}>{count}</Typography>
          {description && <Typography sx={{ fontSize: "11px", color: "#9CA3AF", mt: "5px" }}>{description}</Typography>}
        </Box>
        <Box sx={{ width: 42, height: 42, borderRadius: "11px", backgroundColor: bgColor, display: "flex", alignItems: "center", justifyContent: "center", color, flexShrink: 0 }}>{icon}</Box>
      </Box>
    </Paper>
  );
}

function TicketCard({ ticket, onValider, onRejeter, validating }) {
  const technicien = ticket.technicienId;
  const [note, setNote] = useState(0);
  const canValider = note > 0;
  const priorityColor = PRIORITY_BORDER[ticket.priorite] || "#E5E7EB";

  const solution = ticket.notes?.find(n => n.type === "solution")?.texte
    || ticket.notes?.[ticket.notes.length - 1]?.texte
    || "Aucune solution renseignée";

  return (
    <Paper elevation={0} sx={{ borderRadius: "14px", border: "1px solid #E5E7EB", borderTop: `3px solid ${priorityColor}`, backgroundColor: "#FFFFFF", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", overflow: "hidden", display: "flex", flexDirection: "column", transition: "transform 0.2s, box-shadow 0.2s", "&:hover": { transform: "translateY(-2px)", boxShadow: "0 8px 24px rgba(0,0,0,0.09)" } }}>
      <Box sx={{ padding: "18px 20px", display: "flex", flexDirection: "column", flex: 1 }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: "14px" }}>
          <Box sx={{ flex: 1, pr: 2 }}>
            <Typography sx={{ fontWeight: 700, fontSize: "14px", color: "#111827", mb: "3px", lineHeight: 1.35 }}>{ticket.titre}</Typography>
            <Typography sx={{ fontSize: "11px", color: "#9CA3AF" }}>#{(ticket._id || "").toString().slice(-6).toUpperCase()} · {ticket.localisation}</Typography>
          </Box>
        ...
        <Box sx={{ display: "flex", gap: "12px", mb: "14px" }}>
          <Box sx={{ flex: 1, display: "flex", alignItems: "center", gap: "8px" }}>
            <Box sx={{ width: 30, height: 30, borderRadius: "8px", backgroundColor: "#EFF6FF", display: "flex", alignItems: "center", justifyContent: "center", color: "#2563EB", flexShrink: 0 }}>{Icon.person}</Box>
            <Box>
              ...
              <Typography sx={{ fontSize: "13px", color: "#111827", fontWeight: 600 }}>{technicien?.nom || "Non assigné"}</Typography>
            </Box>
          </Box>
          <Box sx={{ flex: 1, display: "flex", alignItems: "center", gap: "8px" }}>
            <Box sx={{ width: 30, height: 30, borderRadius: "8px", backgroundColor: "#DCFCE7", display: "flex", alignItems: "center", justifyContent: "center", color: "#22C55E", flexShrink: 0 }}>{Icon.calendar}</Box>
            <Box>
              <Typography sx={{ fontSize: "10px", color: "#9CA3AF", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Date</Typography>
              <Typography sx={{ fontSize: "13px", color: "#111827", fontWeight: 600 }}>{formatDate(ticket.updatedAt)}</Typography>
            </Box>
          </Box>
        </Box>
        <Box sx={{ mb: "14px", padding: "10px 12px", backgroundColor: "#F9FAFB", borderRadius: "8px", border: "1px solid #F3F4F6", flex: 1 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: "6px", mb: "5px" }}>
            ...
            <Typography sx={{ fontSize: "10px", color: "#6B7280", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>Solution appliquée</Typography>
          </Box>
          <Typography sx={{ fontSize: "13px", color: "#374151", lineHeight: 1.6 }}>{solution}</Typography>
        </Box>
        <Box sx={{ mb: "14px", padding: "10px 14px", backgroundColor: canValider ? "#F0FDF4" : "#FFFBEB", borderRadius: "8px", border: `1px solid ${canValider ? "#BBF7D0" : "#FDE68A"}`, transition: "all 0.2s" }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <Box>
              ...
              <Typography sx={{ fontSize: "11px", color: "#9CA3AF", mt: "1px" }}>Évaluez la résolution</Typography>
            </Box>
            <StarRating initialRating={note} onRate={val => setNote(val)} />
          </Box>
          {!canValider && <Typography sx={{ fontSize: "11px", color: "#F59E0B", mt: "6px", fontWeight: 500 }}>Attribuez une note avant de valider</Typography>}
        </Box>
        <Divider sx={{ borderColor: "#F3F4F6", mb: "12px" }} />
        <Box sx={{ display: "flex", gap: "8px" }}>
          <Button label={validating ? "Validation…" : "Valider"} variant="primary" onClick={() => onValider(ticket)} disabled={!canValider || validating} />
          <Button label="Rejeter" variant="danger" onClick={() => onRejeter(ticket)} disabled={validating} />
        </Box>
      </Box>
    </Paper>
  );
...
export default function ValiderResolutions() {
  ...
  useEffect(() => {
    const fetchResolved = async () => {
      ...
      try {
        ...
        const resolved = (all || []).filter(t => t.statut === "resolved");
        setTicketsList(resolved);
      } catch (err) {
        console.error("Erreur chargement tickets résolus:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // ✅ Valider via API → statut "closed"
  const handleValider = async (ticket) => {
    setValidating(true);
    try {
      await ticketService.validate(ticket._id || ticket.id, "Résolution validée par le manager.");
      setTicketsList(prev => prev.filter(t => (t._id || t.id) !== (ticket._id || ticket.id)));
      setClosedCount(c => c + 1);
      showSuccess(`Ticket #${(ticket._id || "").toString().slice(-6).toUpperCase()} clôturé avec succès ✅`);
    } catch (err) {
      console.error(err);
    } finally {
      setValidating(false);
  ...
  return (
    <Box sx={{ pb: "80px" }}>
      <Box sx={{ mb: 4 }}>
        <Typography sx={{ fontSize: "32px", fontWeight: 800, color: "#0F172A", letterSpacing: "-0.5px", mb: "8px", fontFamily: "'Playfair Display', Georgia, serif", lineHeight: 1.15 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {!loading && ticketsList.length > 0 && (
            <Box sx={{ backgroundColor: "#F1F5F9", borderRadius: "9999px", padding: "4px 12px", border: "1px solid #E2E8F0" }}>
              <Typography sx={{ fontSize: "14px", fontWeight: 600, color: "#0F172A" }}>{ticketsList.length} en attente</Typography>
            </Box>
          )}
        </Box>
      </Box>
      ...
      {successMsg && (
        <Box sx={{ mb: "16px", padding: "12px 18px", backgroundColor: "#F0FDF4", borderRadius: "10px", border: "1px solid #BBF7D0", display: "flex", alignItems: "center", gap: "10px" }}>
          <Box sx={{ color: "#22C55E", display: "flex" }}>{Icon.check}</Box>
          <Typography sx={{ fontSize: "13px", fontWeight: 600, color: "#15803D" }}>{successMsg}</Typography>
          <Box onClick={() => setSuccessMsg("")} sx={{ ml: "auto", cursor: "pointer", color: "#9CA3AF", fontSize: "16px" }}>×</Box>
        </Box>
      )}

      <Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "14px", marginBottom: "20px" }}>
        <KpiCard icon={Icon.clock}  label="En attente de validation" count={loading ? "…" : ticketsList.length} color="#F59E0B" bgColor="#FFFBEB" description="Tickets à examiner" />
      ...
      <Paper elevation={0} sx={{ borderRadius: "14px", border: "1px solid #E5E7EB", backgroundColor: "#FFFFFF", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", overflow: "hidden" }}>
        <Box sx={{ padding: "16px 24px 0", borderBottom: "1px solid #F3F4F6" }}>
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: "12px" }}>
            <Box>
              <Typography sx={{ fontWeight: 700, fontSize: "15px", color: "#111827" }}>Résolutions à valider</Typography>
              <Typography sx={{ fontSize: "12px", color: "#9CA3AF", mt: "1px" }}>
                {loading ? "Chargement…" : `${filteredTickets.length} ticket${filteredTickets.length !== 1 ? "s" : ""} en attente`}
              </Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: "4px", color: "#9CA3AF" }}>
              {Icon.filter}
              <Typography sx={{ fontSize: "11px", fontWeight: 600, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.06em" }}>Priorité</Typography>
            </Box>
          </Box>
          ...
          <Box sx={{ display: "flex", gap: "4px" }}>
            {FILTER_TABS.map(tab => {
              ...
              const count = tab.key === "all" ? ticketsList.length : tab.key === "critical" ? criticalCount : tab.key === "high" ? highCount : tab.key === "medium" ? mediumCount : lowCount;
              return (
        ...
        <Box sx={{ padding: "20px" }}>
          {loading ? (
            ...
            <Box sx={{ textAlign: "center", padding: "60px 24px" }}>
              ...
              <Typography sx={{ fontWeight: 700, fontSize: "16px", color: "#111827", mb: "6px" }}>Aucun ticket à valider</Typography>
              <Typography sx={{ fontSize: "13px", color: "#9CA3AF" }}>Tous les tickets résolus ont été traités !</Typography>
            </Box>
          ) : (
            <Box sx={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "16px" }}>
              {filteredTickets.map(ticket => (
                <TicketCard key={ticket._id || ticket.id} ticket={ticket} onValider={handleValider} onRejeter={handleRejeter} validating={validating} />
              ))}
            </Box>
          )}
        </Box>
      ...
      >
        ...
        <TextField autoFocus multiline rows={4} fullWidth variant="outlined" label="Raison du rejet"
          ...
          value={raisonRejet} onChange={e => setRaisonRejet(e.target.value)}
          sx={{ mb: "16px", "& .MuiOutlinedInput-root": { borderRadius: "8px", fontSize: "14px" } }}
        />
