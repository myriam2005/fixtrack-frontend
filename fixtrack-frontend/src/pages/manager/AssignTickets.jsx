...
// ✅ UX polish :
//   - Modal pop-up animé avec spring (même thème que le reste)
//   - Toast en bas-droite (cohérent avec le reste de l'app)
//   - Bouton "Passer à la validation" pour tickets resolved (non encore closed)
//   - Pas de bouton assigner/réassigner pour resolved/closed
//   - Bouton quick-assign dans notif simplifié
...
// Statuts pour lesquels on n'affiche PAS de bouton assigner
const NON_ASSIGNABLE = ["resolved", "closed"];
...
if (typeof document !== "undefined" && !document.getElementById("at-styles-v2")) {
  ...
  s.textContent = `
    ...
    .at-assign-btn {
      transition: background 0.15s, transform 0.12s, box-shadow 0.12s, opacity 0.15s !important;
    }
    .at-assign-btn:hover:not(:disabled) {
    ...
    .at-validate-btn {
      transition: background 0.15s, transform 0.12s, box-shadow 0.12s !important;
    }
    .at-validate-btn:hover {
      transform: scale(1.04) !important;
    ...
    .at-tc-card:hover:not(.at-tc-disabled) {
      transform: translateY(-1px);
      box-shadow: 0 4px 14px rgba(37,99,235,0.10);
    }
    .at-tc-card.at-tc-selected {
      border-color: #2563EB !important;
      background: #EFF6FF !important;
    }
    .at-tc-card.at-tc-disabled {
...
function AssignModal({ ticket, techniciens, tickets, onClose, onConfirm, assigning }) {
  const [selectedTech, setSel]   = useState(
    ticket?.technicienId?._id || ticket?.technicienId || null
  );
  ...
  const handleConfirm = () => {
    if (!selectedTech || assigning) return;
    onConfirm(selectedTech, () => handleClose());
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        animation: "at-backdrop-in 0.18s ease",
      }}
    >
      <div style={{
        ...
        boxShadow: "0 24px 80px rgba(10,12,30,0.22), 0 2px 8px rgba(10,12,30,0.08)",
        overflow: "hidden",
        animation: closing ? "at-modal-out 0.22s cubic-bezier(.4,0,1,1) forwards" : "at-modal-in 0.26s cubic-bezier(.22,1,.36,1)",
      }}>
        ...
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #F1F5F9" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ margin: "0 0 4px", fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.7px" }}>
                {isRefused ? "Réassignation" : "Assignation"}
              </p>
              <h2 style={{ margin: 0, fontSize: 17, fontWeight: 800, color: "#0F172A", lineHeight: 1.25 }}>
                Choisir un technicien
              </h2>
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 10, flexWrap: "wrap" }}>
                <span style={{ fontFamily: "monospace", fontSize: 11, color: "#94A3B8", fontWeight: 700, background: "#F1F5F9", padding: "2px 7px", borderRadius: 5 }}>
                  #{(ticket._id || ticket.id || "").toString().slice(-6).toUpperCase()}
                </span>
                <span style={{ fontSize: 13, color: "#374151", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 200 }}>
                  {ticket.titre}
                </span>
            ...
            <button
              ...
              onMouseOut={e => { e.currentTarget.style.background = "#F9FAFB"; e.currentTarget.style.color = "#64748B"; }}
            >
              ✕
            </button>
          </div>
        ...
        <div style={{ overflowY: "auto", padding: "12px 20px 4px", display: "flex", flexDirection: "column", gap: 7, flexGrow: 1 }}>
          {techniciens.map((tech, i) => {
            ...
            const isRefuser = ticket.refusedBy &&
              (ticket.refusedBy?._id || ticket.refusedBy)?.toString() === techId?.toString();
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
                  <div style={{
                    ...
                    background: isRefuser ? "#EF4444" : isSel ? "#2563EB" : AVATAR_COLORS[i % AVATAR_COLORS.length],
                  ...
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 13.5, color: "#0F172A", display: "flex", alignItems: "center", gap: 6 }}>
                      {tech.nom}
                      {isRefuser && (
                        <span style={{ fontSize: 10, fontWeight: 700, color: "#DC2626", background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 4, padding: "1px 5px" }}>
                          A refusé
                        </span>
                      )}
                    </div>
        ...
        <div style={{ padding: "14px 24px 20px", borderTop: "1px solid #F1F5F9", display: "flex", justifyContent: "flex-end", gap: 10 }}>
          ...
          <Button
            label={assigning ? "En cours…" : isRefused ? "Confirmer la réassignation" : "Confirmer l'assignation"}
            variant="primary"
...
function Toast({ toast }) {
  ...
  return (
    <div style={{
      ...
      animation: visible ? "at-toast-in 0.22s cubic-bezier(.22,1,.36,1)" : "at-toast-out 0.2s ease forwards",
      pointerEvents: "none",
    }}>
      <span style={{
        width: 8, height: 8, borderRadius: "50%", flexShrink: 0,
        background: isError ? "#FCA5A5" : isWarn ? "#FCD34D" : "#22C55E",
      }} />
      {toast.msg}
    </div>
  );
...
function TicketRow({ ticket, tech, isLast, onAssign, onGoValidate, index }) {
  ...
  return (
    <>
      <Box
        className={`at-row${isRefused ? " refused-row" : ""}`}
        sx={{
          ...
          animation: `at-row-in 0.3s ease ${index * 0.04}s both`,
      ...
      >
        ...
        <Box sx={{ width: { xs: "auto", md: 44 }, flexShrink: 0 }}>
          <span style={{ fontFamily: "monospace", fontSize: 11.5, color: "#64748B", fontWeight: 600 }}>
            {(ticket._id || ticket.id || "").toString().slice(-6).toUpperCase()}
          </span>
        </Box>
        ...
        <Box sx={{ width: { xs: "auto", md: 150 }, flexShrink: 0 }}>
          {techName ? (
            <div className={styles.techWrap}>
              <div className={styles.avatar} style={{ background: isRefused ? "#EF4444" : AVATAR_COLORS[0] }}>
        ...
        <Box sx={{ flexShrink: 0, width: { xs: "100%", md: 160 }, display: "flex", justifyContent: { xs: "flex-end", md: "center" } }}>
          {isClosed ? (
            // Ticket clôturé — indicateur discret
            ...
            // ✅ Ticket résolu mais pas encore validé → bouton "Passer à la validation"
            <button
              className="at-validate-btn"
...
export default function AssignTicket() {
  ...
  // ✅ Ouvre le modal si ?ticketId= est dans l'URL (depuis notif quick-assign)
  useEffect(() => {
  ...
  // Adapter le path selon votre routeur (ex: "/manager/valider-resolutions")
  const handleGoValidate = () => {
    navigate("/ValiderResolutions");
  };
      ...
      <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #E2E8F0", boxShadow: "0 1px 6px rgba(0,0,0,0.05)", overflow: "hidden" }}>
        ...
        <Box sx={{ display: "flex", alignItems: { xs: "flex-start", sm: "center" }, flexDirection: { xs: "column", sm: "row" }, justifyContent: "space-between", gap: "12px", padding: { xs: "16px 16px 12px", md: "20px 24px 16px" } }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: "8px", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: "10px", padding: "8px 14px", width: { xs: "100%", sm: "280px" }, "&:focus-within": { borderColor: "#2563EB", boxShadow: "0 0 0 3px rgba(37,99,235,0.08)" }, transition: "all 0.15s" }}>
            ...
            <input
              type="text" placeholder="Rechercher un ticket…" value={search}
        ...
        <Box sx={{ display: "flex", padding: { xs: "0 12px", md: "0 24px" }, borderBottom: "1px solid #F1F5F9", overflowX: "auto", "&::-webkit-scrollbar": { display: "none" } }}>
          {TABS.map(tab => {
            ...
            const isRef    = tab.key === "refused";
            const isRes    = tab.key === "resolved";
            return (
              <button
                key={tab.key} className="at-tab"
                ...
                style={{
                  ...
                  color: isActive
                    ? (isRef ? "#DC2626" : isRes ? "#16A34A" : "#2563EB")
                    : isRef && count > 0 ? "#DC2626"
                    : isRes && count > 0 ? "#16A34A"
                    : "#64748B",
                  borderBottom: isActive
                    ? `2.5px solid ${isRef ? "#DC2626" : isRes ? "#16A34A" : "#2563EB"}`
              ...
              >
                ...
                <span style={{
                  background: isActive
                    ? (isRef ? "#DC2626" : isRes ? "#16A34A" : "#2563EB")
                    : isRef && count > 0 ? "#FEF2F2"
                    : isRes && count > 0 ? "#F0FDF4"
                    : "#E2E8F0",
                  color: isActive ? "#fff"
                    : isRef && count > 0 ? "#DC2626"
                    : isRes && count > 0 ? "#16A34A"
                  ...
                  border: isRef && count > 0 && !isActive ? "1px solid #FECACA"
                    : isRes && count > 0 && !isActive ? "1px solid #BBF7D0"
                    : "none",
                }}>
                  {count}
                </span>
        ...
        <Box sx={{ display: { xs: "none", md: "flex" }, alignItems: "center", gap: "14px", padding: "10px 18px 10px 16px", borderBottom: "1px solid #F1F5F9", background: "#FAFBFC" }}>
          {["", "ID", "Ticket", "Catégorie", "Priorité", "Statut", "Date", "Assigné à", "Action"].map((col, i) => (
            <Box key={i} sx={{ flex: i === 2 ? 1 : undefined, width: [7,44,undefined,110,100,110,90,150,160][i], flexShrink: i !== 2 ? 0 : undefined, display: i === 3 ? { xs: "none", lg: "block" } : "block", textAlign: i === 8 ? "center" : "left", fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.6px" }}>
              {col}
            </Box>
          ))}
        </Box>
        ...
        <Box sx={{ padding: { xs: "4px 2px 12px", md: "4px 6px 12px" } }}>
          {loading ? (
            <Box sx={{ p: "20px", display: "flex", flexDirection: "column", gap: "12px" }}>
              {[1,2,3,4].map(i => (
                <Box key={i} sx={{ height: 60, borderRadius: 8, background: "#F1F5F9", animation: "pulse 1.5s ease-in-out infinite", "@keyframes pulse": { "0%,100%": { opacity: 1 }, "50%": { opacity: 0.5 } } }} />
              ))}
            </Box>
          ) : visible.length === 0 ? (
            ...
            <TicketRow
              ...
              index={index}
              onAssign={t => setModal(t)}
              onGoValidate={handleGoValidate}
            />
          ))}
        </Box>
      </div>
