...
// ── Icônes locales (celles non présentes dans Icons.jsx) ───────────────────────
const Icon = {
  ...
  // Réutilisés depuis Icons.jsx via composants wrappés inline pour garder la même API {Icon.xxx}
      ...
      <Box sx={{ display:"flex", alignItems:"center", gap:"24px", flexWrap:"wrap" }}>
        <Box sx={{ flexShrink:0, position:"relative" }}>
          ...
          <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} style={{ overflow:"visible" }}>
            ...
            <text x={CX} y={CY - 10} textAnchor="middle" fontSize={hovSlice ? "18" : "24"} fontWeight="900" fill={hovSlice ? hovSlice.color : "#111827"} style={{ transition:"all 0.25s ease", fontVariantNumeric:"tabular-nums" }}>
              {hovSlice ? hovSlice.value : total}
            </text>
            <text x={CX} y={CY + 8} textAnchor="middle" fontSize="9" fontWeight="700" fill={hovSlice ? hovSlice.color : "#9CA3AF"} style={{ textTransform:"uppercase", letterSpacing:"0.07em", transition:"all 0.25s ease" }}>
              {hovSlice ? hovSlice.name.split(" ")[0] : "tickets"}
            </text>
            {hovSlice && (
              <text x={CX} y={CY + 24} textAnchor="middle" fontSize="13" fontWeight="800" fill={hovSlice.color}>
                {hovSlice.pct}%
              </text>
            )}
          </svg>
          ...
          {slices.map((s, i) => (
            <Box
              ...
              sx={{
                ...
                padding:"7px 10px", borderRadius:"10px", border:"1.5px solid",
            ...
            >
              <Box sx={{ display:"flex", alignItems:"center", gap:"8px" }}>
                ...
                <Typography sx={{ fontSize:"12px", fontWeight: hovered === i ? 700 : 500, color: hovered === i ? "#111827" : "#374151", maxWidth:"100px", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", transition:"font-weight 0.15s ease, color 0.15s ease" }}>
                  {s.name}
                </Typography>
              </Box>
              <Box sx={{ display:"flex", alignItems:"center", gap:"6px", ml:"8px" }}>
                <Box sx={{ backgroundColor: hovered === i ? s.color : "#F3F4F6", color: hovered === i ? "#fff" : "#6B7280", borderRadius:"6px", padding:"1px 7px", fontSize:"11px", fontWeight:800, transition:"background-color 0.2s ease, color 0.2s ease" }}>
                  {s.value}
                </Box>
                <Typography sx={{ fontSize:"10px", fontWeight:700, minWidth:"28px", color: hovered === i ? s.color : "#9CA3AF", transition:"color 0.2s ease" }}>
                  {s.pct}%
                </Typography>
              </Box>
            </Box>
