...
function UnassignedUrgentPanel({ unassignedTickets, techniciens }) {
  ...
  return (
    <Box sx={{ mt: "14px", display: "flex", flexDirection: "column", gap: "8px" }}>
      {unassignedTickets.map((ticket) => {
        ...
        return (
          <Box key={ticket.id} sx={{
            ...
            padding: "10px 14px", transition: "all 0.18s",
            "&:hover": { backgroundColor: `${cfg.color}10`, border: `1.5px solid ${cfg.color}55`, transform: "translateX(3px)" },
          }}>
      ...
      <Box sx={{ mt: "4px", borderRadius: "10px", padding: "10px 14px", backgroundColor: "#F0F7FF", border: "1px dashed #BFDBFE", display: "flex", alignItems: "center", gap: "8px" }}>
...
function TechComparisonPanel({ techniciens, allTickets }) {
  ...
  const data = useMemo(() => {
    return techniciens.map((tech) => {
      ...
      return { id: tech.id, nom, actifs, resolus, total, score, competences: tech.competences || [], chargeLevel };
    });
            ...
            <Box key={tech.id} onClick={() => setSelected(isSel ? null : tech.id)} sx={{
              ...
              <Box sx={{ mb: "8px" }}>
                ...
                <Box sx={{ height: 5, borderRadius: "999px", backgroundColor: "#E5E7EB", overflow: "hidden" }}>
                  <Box sx={{ width: `${barPct}%`, height: "100%", background: `linear-gradient(90deg, ${cfg.bar}, ${cfg.bar}BB)`, borderRadius: "999px", transition: "width 0.6s cubic-bezier(0.34,1.56,0.64,1)" }} />
                </Box>
              </Box>
  ...
  return (
    <Box sx={{ mt: "14px", display: "flex", flexDirection: "column", gap: "8px" }}>
      {data.map((item) => {
        ...
        return (
          <Box key={item.cat} onMouseEnter={() => setHovered(item.cat)} onMouseLeave={() => setHovered(null)} sx={{
