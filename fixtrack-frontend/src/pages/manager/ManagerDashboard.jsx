...
// ── Feature 2 : Comparaison charge techniciens (visuel côte-à-côte) ────────────
function TechComparisonPanel({ techniciens, allTickets }) {
  const [selected, setSelected] = useState(null);

  const data = useMemo(() => {
    return techniciens.map((tech) => {
      const actifs = allTickets.filter(
        (t) => t.technicienId === tech.id && ["assigned","in_progress"].includes(t.statut)
      ).length;
      const resolus = allTickets.filter(
        (t) => t.technicienId === tech.id && ["resolved","closed"].includes(t.statut)
      ).length;
      ...
      // Charge : 0–3 actifs = OK, 4–6 = chargé, 7+ = surchargé
            ...
            >
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: "8px", mb: "10px" }}>
                <Avatar sx={{
                  ...
                  color: isSel ? "#FFFFFF" : cfg.bar,
                  transition: "all 0.18s",
                }}>
                  {tech.nom[0]}
                </Avatar>
                <Box>
                  <Typography sx={{ fontSize: "12px", fontWeight: 700, color: "#111827", lineHeight: 1 }}>
                    {tech.nom}
                  </Typography>
                  <Box sx={{ backgroundColor: cfg.badge, borderRadius: "4px", padding: "1px 5px", mt: "2px", display: "inline-block" }}>
                    <Typography sx={{ fontSize: "9px", fontWeight: 700, color: cfg.text }}>
                      {cfg.label}
                    </Typography>
                  </Box>
                </Box>
              ...
              <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <Box sx={{
                  ...
                  <Typography sx={{ fontSize: "10px", fontWeight: 700, color: "#374151" }}>
                    {tech.score}% résol.
                  </Typography>
                </Box>
              </Box>

              {/* Compétences (si sélectionné) */}
              {isSel && tech.competences.length > 0 && (
                <Box sx={{ mt: "8px", pt: "8px", borderTop: "1px solid #F3F4F6" }}>
                  <Typography sx={{ fontSize: "9px", color: "#9CA3AF", mb: "4px", textTransform: "uppercase", fontWeight: 600 }}>
                    Compétences
                  </Typography>
...
// ── Ligne ticket prioritaire ───────────────────────────────────────────────────
function PriorityTicketRow({ ticket, techniciens, isLast }) {
...
export default function MgrDashboard() {
  ...
  // Tickets non assignés urgents (critical ou high, sans technicien)
  const unassignedUrgent = useMemo(() => {
    const ORDER = { critical: 0, high: 1 };
    return [...allTickets]
      .filter((t) =>
        !t.technicienId &&
        ["critical","high"].includes(t.priorite) &&
  ...
  // Tickets haute priorité pour le tableau
  const highPriorityTickets = useMemo(() => {
    const ORDER = { critical: 0, high: 1 };
    return [...allTickets]
      .filter((t) => ["critical","high"].includes(t.priorite) && !["resolved","closed"].includes(t.statut))
      .sort((a, b) => (ORDER[a.priorite]??9) - (ORDER[b.priorite]??9) || new Date(b.dateCreation) - new Date(a.dateCreation))
      .slice(0, 6);
  }, [allTickets]);

  const FILTER_TABS = [
    { key: "all",        label: "Tous",           count: highPriorityTickets.length },
    { key: "critical",   label: "Critiques",      count: highPriorityTickets.filter((t) => t.priorite === "critical").length },
    { key: "high",       label: "Haute priorité", count: highPriorityTickets.filter((t) => t.priorite === "high").length },
    { key: "unassigned", label: "Non assignés",   count: highPriorityTickets.filter((t) => !t.technicienId).length },
  ];

  const filteredTickets = useMemo(() => {
    if (activeFilter === "critical")   return highPriorityTickets.filter((t) => t.priorite === "critical");
    if (activeFilter === "high")       return highPriorityTickets.filter((t) => t.priorite === "high");
    if (activeFilter === "unassigned") return highPriorityTickets.filter((t) => !t.technicienId);
    return highPriorityTickets;
  }, [highPriorityTickets, activeFilter]);
  ...
  return (
    <Box sx={{ pb: "80px" }}>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", mb: "20px" }}>
        ...
        <Paper elevation={0} sx={{
          ...
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            {unassignedUrgent.length > 0 && (
              <Box sx={{
                backgroundColor: "#FEE2E2", borderRadius: "20px",
        ...
        {/* Comparaison charge techniciens */}
        <Paper elevation={0} sx={{
          borderRadius: "16px", padding: "20px 22px",
      ...
      {/* ── Tableau tickets haute priorité ── */}
      <Paper elevation={0} sx={{
            ...
            <Box>
              <Box sx={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <Typography sx={{ fontWeight: 700, fontSize: "15px", color: "#111827" }}>
                  Tickets haute priorité
                </Typography>
                {filteredTickets.length > 0 && (
                  <Box sx={{ backgroundColor: "#FEE2E2", color: "#DC2626", borderRadius: "20px", padding: "1px 8px", fontSize: "11px", fontWeight: 700 }}>
                    {filteredTickets.length} actifs
                  </Box>
                )}
              </Box>
              <Typography sx={{ fontSize: "12px", color: "#9CA3AF", mt: "1px" }}>
                Critiques et hautes priorités en attente de résolution
              </Typography>
            </Box>
            <Link to="/manager/tickets" style={{ textDecoration: "none" }}>
        ...
        <Box sx={{ padding: "8px 6px 12px" }}>
          {filteredTickets.length === 0 ? (
            <Box sx={{ textAlign: "center", padding: "40px 24px" }}>
              ...
              <Typography sx={{ fontSize: "12px", color: "#9CA3AF" }}>Toutes les urgences sont traitées</Typography>
            </Box>
          ) : (
