...
export default function LoginPage({ onSwitchToSignup, onLoginSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ AuthContext login (remplace localStorage direct)
    login({
      name: account.name,
      role: account.role,
  ...
  return (
    <div className={styles.authPage}>
        ...
        <div className={styles.formPanel}>
          <div className={styles.formInner}>
            ...
            <div className={styles.demoHint}>
              💡 <strong>Démo</strong> — MDP universel :{" "}
              <code>fixtrack2025</code>
              <br />
              <span style={{ fontSize: 11 }}>
                employee@ · tech@ · manager@ · admin@{" "}
                <em>(@fixtrack.app)</em>
              </span>
            </div>
            ...
            <Divider sx={{ mb: 2 }}>
              <span
                style={{ fontSize: 12, color: "#94a3b8", padding: "0 8px" }}
              >
                ou
              </span>
            </Divider>
      ...
      <div className={styles.pageFooter}>
        <span className={styles.pageFooterItem}>
          © 2024 FixTrack. All rights reserved.
        </span>
        {["Terms of Service", "Privacy Policy", "Support"].map((t) => (
          <span
            key={t}
            className={`${styles.pageFooterItem} ${styles.pageFooterLink}`}
          >
            {t}
          </span>
        ))}
