...
// Fix: suppression de la variable 'loading' non utilisée
    ...
    <Box sx={{ pb: "80px" }}>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1.1fr 1.2fr", gap: "16px", mb: "16px" }}>
        ...
        <Paper elevation={0} sx={{ borderRadius: "14px", border: "1px solid #E5E7EB", backgroundColor: "#fff", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", overflow: "hidden" }}>
          ...
          <Box sx={{ padding: "6px 6px 8px" }}>
            {urgentTickets.map((t, i) => <UrgentRow key={t.id || t._id} ticket={t} isLast={i === urgentTickets.length - 1} />)}
          </Box>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "1.3fr 1fr 1fr", gap: "16px" }}>
        <Paper elevation={0} sx={{ borderRadius: "14px", border: "1px solid #E5E7EB", backgroundColor: "#fff", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", overflow: "hidden" }}>
          ...
          />
          <Box>
            {users.map((u, i) => <UserRow key={u.id || u._id} user={u} isLast={i === users.length - 1} />)}
          </Box>
        </Paper>
        ...
        <Paper elevation={0} sx={{ borderRadius: "14px", border: "1px solid #E5E7EB", backgroundColor: "#fff", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", overflow: "hidden" }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 18px" }}>
            ...
            <Typography sx={{ fontSize: "13px", fontWeight: 700, color: "#22C55E" }}>{techWorkload.filter((t) => t.assigned === 0).length} / {technicians.length}</Typography>
          </Box>
        </Paper>
        ...
        <Paper elevation={0} sx={{ borderRadius: "14px", border: "1px solid #E5E7EB", backgroundColor: "#fff", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", overflow: "hidden" }}>
          ...
          <Box sx={{ borderTop: "1px solid #F3F4F6", padding: "9px 20px", textAlign: "center" }}>
            <Typography sx={{ fontSize: "12px", fontWeight: 600, color: "#2563EB", cursor: "pointer", "&:hover": { textDecoration: "underline" } }}>Télécharger le journal complet →</Typography>
          </Box>
        </Paper>
      </Box>
