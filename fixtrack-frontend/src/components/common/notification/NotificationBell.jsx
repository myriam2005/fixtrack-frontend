...
// ✅ Inchangé structurellement — utilise le NotificationItem mis à jour
...
// ── Wrapper avec bouton supprimer au hover ────────────────────────────────────
function NotifItem({ notif, onRead, onRemove }) {
  // Adapte le format NotificationContext → NotificationItem
  const adapted = {
    _id:       notif.id,
  ...
  return (
    <Box sx={{ position: "relative", "&:hover .nb-del": { opacity: 1, visibility: "visible" } }}>
      <NotificationItem notif={adapted} onMarkRead={onRead} />
      <IconButton
        className="nb-del"
        size="small"
        onClick={e => { e.stopPropagation(); onRemove && onRemove(notif.id); }}
        sx={{
          ...
          "&:hover": { backgroundColor: "#FEF2F2", borderColor: "#FECACA", color: "#DC2626" },
        }}
      >
        <TrashIcon />
      </IconButton>
...
export default function NotificationBell() {
  const { getNotificationsForUser, getUnreadCountForUser, markAllAsRead, markAsRead, removeNotification } = useNotifications();
  const { user } = useAuth();
    ...
    <Box ref={ref} sx={{ position: "relative" }}>
      <Tooltip title="Notifications" placement="bottom">
        <IconButton onClick={() => setOpen(v => !v)} size="small" sx={bellButtonSx(open)}>
          <BellIcon />
          {unreadCount > 0 && (
            <Box sx={badgeSx}>{unreadCount > 99 ? "99+" : unreadCount}</Box>
          )}
        </IconButton>
      </Tooltip>
          ...
          <Box sx={headerSx}>
            ...
            {unreadCount > 0 && (
              <Box onClick={() => markAllAsRead(userId)} sx={markAllBtnSx}>
                <Box sx={{ color: "#64748B", display: "flex" }}><CheckAllIcon /></Box>
                <Typography sx={{ fontSize: 11, fontWeight: 500, color: "#64748B" }}>Tout lire</Typography>
              </Box>
            )}
          </Box>
          ...
          <Box sx={listContainerSx}>
            {notifications.length === 0 ? (
              <Box sx={emptyStateSx}>
                ...
                <Typography sx={{ fontSize: 13, color: "#94A3B8", fontWeight: 500 }}>Aucune notification</Typography>
                <Typography sx={{ fontSize: 11, color: "#CBD5E1", mt: 0.5 }}>Vous êtes à jour</Typography>
              </Box>
            ) : (
