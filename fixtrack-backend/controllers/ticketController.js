  ...
  try {
    ...
    if (statut === "in_progress" && oldStatut === "resolved") {
      ...
      if (techId)
        await sendNotification({
          userId: techId,
          message: `⚠️ ${req.user?.nom || "Le manager"} a rejeté votre résolution sur le ticket "${ticket.titre}". Veuillez reprendre l'intervention.`,
          type: "status_changed",
          ticketId: ticket._id,
        });
    }
...
exports.saveFeedback = async (req, res) => {
  try {
    ...
    const auteurId = ticket.auteurId?.toString();
    ...
    if (isFirstFeedback) {
      ...
      ticket.notes.push({
        auteur: req.user.id,
        texte: `FEEDBACK: ${labels[Number(rating)]} (${rating}/5)${comment?.trim() ? ` — "${comment.trim()}"` : ""}`,
        type: "validation",
        date: new Date(),
      });
    }
    ...
    await createLog(
      "FEEDBACK_ADDED",
      `Feedback ${rating}/5 sur "${ticket.titre}"`,
      req.user.id,
    );
    ...
    res.json({ message: "Feedback enregistré", ticket: updated });
  } catch (err) {
